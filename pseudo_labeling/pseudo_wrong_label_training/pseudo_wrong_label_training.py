# ============================================================================
# CELL 10 v8c: v6 + AGREEMENT-FILTERED PSEUDO-LABELS (v3+v6 both ≥0.7)
# ============================================================================
# GT data (561): classification loss + embedding loss
# Pseudo data (~1393): classification loss ONLY (soft label, weight=0.5)
# Agreement filtering: both v3 and v6 must agree ≥0.7
# ============================================================================
...
PSEUDO_PATH = os.path.join(DATA_ROOT, 'pseudo_labels_noise.csv')
SAVE_DIR = os.path.join(DATA_ROOT, 'results_distillation_v8_noise')
...
# Convert pseudo probabilities to soft labels (agreement avg)
for sn in ALL_SHORT:
    pseudo_df[f'gt_{sn}'] = pseudo_df[f'prob_avg_{sn}'].values  # avg of v3+v6

# Filter: keep only images that exist
pseudo_df = pseudo_df[pseudo_df['image_path'].apply(os.path.exists)].reset_index(drop=True)

# Only keep samples with at least 1 agreed label
...
# ─────────────────────────────────────────────
print(f"\n{'='*80}")
print(f"🚀 TRAINING v8c — Agreement-Filtered Pseudo-Labels ({DEVICE})")
...
for fold, (train_idx, val_idx) in enumerate(splits):
    ...
    for sn, col in zip(RARE_SHORT, RARE_LABELS):
        ...
        ps_n = int((pseudo_df[f'prob_avg_{sn}'] >= 0.7).sum())
...
# ═══════════════════════════════════════════════════════════════
print(f"\n{'='*80}")
print(f"📊 GLOBAL RESULTS — v8c (Agreement-Filtered Pseudo-Labels)")
print(f"{'='*80}")
...
gmacro = print_table(gm, title="GLOBAL METRICS (v8c)")
...
print(f"\n  {'Lab':5s} {'N':>3s} {'v3':>6s} {'v6':>6s} {'v8c':>6s} {'Δv6':>7s} {'Best':>5s}")
print(f"  {'─'*42}")
for i,sn in enumerate(ALL_SHORT):
    d3=v3_f1.get(sn,0);d6=v6_f1.get(sn,0);d8c=f1s[i]
    delta=d8c-d6; best=max(d3,d6,d8c)
    mk="v8c" if d8c>=best-0.001 else("v6" if d6>=best-0.001 else "v3")
    print(f"  {sn:5s} {gm[i]['N']:3d} {d3:6.3f} {d6:6.3f} {d8c:6.3f} {delta:+7.3f} {mk:>5s}")
macro=np.mean(f1s)
print(f"  {'─'*42}")
print(f"  {'MACRO':5s} {'':3s} {'.795':>6s} {'.811':>6s} {macro:6.3f} {macro-.811:+7.3f}")

if macro > 0.85:
    print(f"\n  ✅ v8c > 0.85 — agreement filtering works!")
elif macro > 0.811:
    print(f"\n  🟡 v8c > v6 — improvement confirmed")
else:
    print(f"\n  ⚠️ v8c ≤ v6 — agreement pseudo-labels not helping")

# Save
res={'macro_f1':float(macro),'macro_auc':float(gmacro['AUC']),
     'gt_samples':len(meta),'pseudo_samples':len(pseudo_df),
     'pseudo_weight':PSEUDO_WEIGHT,'pseudo_approach':'agreement_v3_v6',
