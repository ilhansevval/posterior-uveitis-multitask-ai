# ============================================================================
# PATOLOJI EMBEDDING QUALITY ANALYSIS — Phi-3.5-Vision + Reference
# ============================================================================
...
EMB_DIR = os.path.join(DATA_ROOT, 'teacher_embeddings_phi')   # ★ Phi
...
# GLOBAL METRICS
print(f"\n{'='*70}\n📊 GLOBAL METRICS — Phi-3.5-Vision + Ref\n{'='*70}")
...
# COMPARISON
print(f"\n{'='*70}\n📊 Phi vs MedGemma vs Qwen+Ref COMPARISON\n{'='*70}")
...
print(f"\n  {'Metric':<22s} {'Phi':>8s} {'MedGemma':>9s} {'Qwen+Ref':>9s}")
print(f"  {'─'*55}")
print(f"  {'Global multi-gap':<22s} {PHI['global_gap']:>+8.4f} "
      f"{MEDGEMMA['global_gap']:>+9.4f} {QWEN_REF['global_gap']:>+9.4f}")
print(f"  {'NN-1 exact match':<22s} {PHI['nn1_exact']:>8.4f} "
...
for grp_name, grp_key in [('Common', 'macro_common'), ('Rare', 'macro_rare'), ('All-8', 'macro_all')]:
    ...
    for m in ['gap', 'nn1', 'lp_f1', 'lp_auc', 'lp_ap']:
        ...
        print(f"  {m:<22s} {p:>8.4f} {mg:>9.4f} {qr:>9.4f}")
    print()
...
print(f"  {'Phi-3.5-Vision + Ref':<22s} {'Gen.VLM(DP)':<14s} {'4.2B':>6s} {'✗':>4s} {'✗':>4s} "
      f"{PHI['macro_all']['lp_f1']:>8.3f} "
      f"{PHI['macro_rare']['lp_f1']:>8.3f} "
      f"{PHI['macro_all']['lp_auc']:>7.3f} "
      f"{PHI['nn1_exact']:>7.3f} "
      f"{PHI['global_gap']:>+9.4f}")
...
# LATEX
print(f"\n{'='*70}\n📋 LATEX — Per-class detail (Phi)\n{'='*70}\n")

print("\\begin{table}[h]")
print("\\centering")
print("\\caption{Per-class embedding quality for Phi-3.5-Vision + Reference images.}")
print("\\label{tab:emb_quality_phi}")
...
summary = {
    ...
    'macro_all': PHI['macro_all'],
    'macro_rare': PHI['macro_rare'],
    'macro_common': PHI['macro_common'],
    'per_class': results,
}
