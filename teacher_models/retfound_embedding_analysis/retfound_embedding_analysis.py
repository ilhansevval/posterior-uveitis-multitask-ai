# ============================================================================
# PATOLOJI EMBEDDING QUALITY ANALYSIS — RETFound-MAE-CFP
# ============================================================================
...
EMB_DIR = os.path.join(DATA_ROOT, 'teacher_embeddings_retfound_cfp')   # ★ RETFound CFP
...
print(f"\n{'='*70}\n📊 RETFound vs Phi vs MedGemma vs Qwen+Ref\n{'='*70}")
...
print(f"\n  {'Metric':<20s} {'RETFound':>9s} {'Phi':>8s} {'MedGemma':>9s} {'Qwen+Ref':>9s}")
print(f"  {'─'*60}")
print(f"  {'Global multi-gap':<20s} {RETFOUND['global_gap']:>+9.4f} {PHI['global_gap']:>+8.4f} "
      f"{MEDGEMMA['global_gap']:>+9.4f} {QWEN_REF['global_gap']:>+9.4f}")
print(f"  {'NN-1 exact':<20s} {RETFOUND['nn1_exact']:>9.4f} {PHI['nn1_exact']:>8.4f} "
      f"{MEDGEMMA['nn1_exact']:>9.4f} {QWEN_REF['nn1_exact']:>9.4f}")
print(f"  {'─'*60}")
for grp_name, grp_key in [('Common', 'macro_common'), ('Rare', 'macro_rare'), ('All-8', 'macro_all')]:
    ...
    for m in ['gap', 'nn1', 'lp_f1', 'lp_auc', 'lp_ap']:
        ...
        print(f"  {m:<20s} {rf:>9.4f} {p:>8.4f} {mg:>9.4f} {qr:>9.4f}")
    print()

# UNIFIED PAPER TABLE
print(f"\n{'='*70}\n📋 PAPER TABLE — All Models\n{'='*70}\n")
...
# LATEX
print(f"\n{'='*70}\n📋 LATEX — Per-class detail (RETFound-CFP)\n{'='*70}\n")

print("\\begin{table}[h]")
print("\\centering")
print("\\caption{Per-class embedding quality for RETFound-MAE-CFP (encoder-only, no references).}")
print("\\label{tab:emb_quality_retfound}")
...
summary = {
    ...
    'macro_all': RETFOUND['macro_all'],
    'macro_rare': RETFOUND['macro_rare'],
    'macro_common': RETFOUND['macro_common'],
    'per_class': results,
}
