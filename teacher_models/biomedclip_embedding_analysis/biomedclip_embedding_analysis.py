# ============================================================================
# PATOLOJI EMBEDDING QUALITY ANALYSIS — BiomedCLIP
# ============================================================================
...
EMB_DIR = os.path.join(DATA_ROOT, 'teacher_embeddings_biomedclip')   # ★ BiomedCLIP
...
print(f"\n{'='*70}\n📊 BiomedCLIP vs RETFound vs Phi vs MedGemma vs Qwen+Ref\n{'='*70}")
...
QWEN_REF = {
    ...
    'global_gap': 0.0437, 'nn1_exact': 0.8146,
}
MEDGEMMA = {
    'macro_all':    {'gap': 0.0207, 'nn1': 0.6703, 'lp_f1': 0.4372, 'lp_auc': 0.8694, 'lp_ap': 0.5534},
    'macro_rare':   {'gap': 0.0156, 'nn1': 0.4838, 'lp_f1': 0.2517, 'lp_auc': 0.8427, 'lp_ap': 0.3736},
    'macro_common': {'gap': 0.0258, 'nn1': 0.8567, 'lp_f1': 0.6226, 'lp_auc': 0.8961, 'lp_ap': 0.7332},
    'global_gap': 0.0340, 'nn1_exact': 0.8271,
}
PHI = {
    'macro_all':    {'gap': 0.0113, 'nn1': 0.8153, 'lp_f1': 0.5053, 'lp_auc': 0.8393, 'lp_ap': 0.5618},
    'macro_rare':   {'gap': 0.0137, 'nn1': 0.7406, 'lp_f1': 0.3419, 'lp_auc': 0.8309, 'lp_ap': 0.4328},
    'macro_common': {'gap': 0.0090, 'nn1': 0.8900, 'lp_f1': 0.6687, 'lp_auc': 0.8476, 'lp_ap': 0.6908},
    'global_gap': 0.0215, 'nn1_exact': 0.8556,
}
RETFOUND = {
    'macro_all':    {'gap': 0.0052, 'nn1': 0.4445, 'lp_f1': 0.3312, 'lp_auc': 0.7665, 'lp_ap': 0.3472},
    'macro_rare':   {'gap': 0.0073, 'nn1': 0.4020, 'lp_f1': 0.2275, 'lp_auc': 0.8139, 'lp_ap': 0.2304},
    'macro_common': {'gap': 0.0031, 'nn1': 0.4871, 'lp_f1': 0.4349, 'lp_auc': 0.7191, 'lp_ap': 0.4640},
    'global_gap':-0.0046, 'nn1_exact': 0.3993,
}
...
BIOMEDCLIP = {
    ...
    'global_gap': multi_gap, 'nn1_exact': nn1_exact,
}

print(f"\n  {'Metric':<18s} {'BMedCLIP':>9s} {'RETFound':>9s} {'Phi':>8s} {'MedGm':>8s} {'Qwen+R':>8s}")
print(f"  {'─'*70}")
print(f"  {'Global multi-gap':<18s} {BIOMEDCLIP['global_gap']:>+9.4f} {RETFOUND['global_gap']:>+9.4f} "
      f"{PHI['global_gap']:>+8.4f} {MEDGEMMA['global_gap']:>+8.4f} {QWEN_REF['global_gap']:>+8.4f}")
print(f"  {'NN-1 exact':<18s} {BIOMEDCLIP['nn1_exact']:>9.4f} {RETFOUND['nn1_exact']:>9.4f} "
      f"{PHI['nn1_exact']:>8.4f} {MEDGEMMA['nn1_exact']:>8.4f} {QWEN_REF['nn1_exact']:>8.4f}")
print(f"  {'─'*70}")
for grp_name, grp_key in [('Common', 'macro_common'), ('Rare', 'macro_rare'), ('All-8', 'macro_all')]:
    print(f"  -- {grp_name} --")
    for m in ['gap', 'nn1', 'lp_f1', 'lp_auc', 'lp_ap']:
        bc = BIOMEDCLIP[grp_key][m]
        rf = RETFOUND[grp_key][m]
        p = PHI[grp_key][m]
        mg = MEDGEMMA[grp_key][m]
        qr = QWEN_REF[grp_key][m]
        print(f"  {m:<18s} {bc:>9.4f} {rf:>9.4f} {p:>8.4f} {mg:>8.4f} {qr:>8.4f}")
    print()

# UNIFIED PAPER TABLE — 6 models
print(f"\n{'='*90}\n📋 PAPER TABLE — All 6 Models (FINAL)\n{'='*90}\n")

QWEN_NOREF = {
    'lp_f1_all': 0.4438, 'lp_f1_rare': 0.2558,
    'lp_auc': 0.8711, 'nn1': 0.8021, 'gap': 0.0165
}

print(f"  {'Model':<24s} {'Type':<14s} {'Param':>6s} {'Med':>4s} {'Ret':>4s} "
      f"{'F1-all':>8s} {'F1-rare':>8s} {'AUC':>7s} {'NN-1':>7s} {'Gap':>9s}")
print(f"  {'─'*105}")

# Encoder-only baselines first
print(f"  {'BiomedCLIP':<24s} {'CLIP-style':<14s} {'400M':>6s} {'✓':>4s} {'✗':>4s} "
      f"{BIOMEDCLIP['macro_all']['lp_f1']:>8.3f} {BIOMEDCLIP['macro_rare']['lp_f1']:>8.3f} "
      f"{BIOMEDCLIP['macro_all']['lp_auc']:>7.3f} {BIOMEDCLIP['nn1_exact']:>7.3f} {BIOMEDCLIP['global_gap']:>+9.4f}")

print(f"  {'RETFound-CFP':<24s} {'SSL ViT':<14s} {'300M':>6s} {'✓':>4s} {'✓':>4s} "
      f"{RETFOUND['macro_all']['lp_f1']:>8.3f} {RETFOUND['macro_rare']['lp_f1']:>8.3f} "
      f"{RETFOUND['macro_all']['lp_auc']:>7.3f} {RETFOUND['nn1_exact']:>7.3f} {RETFOUND['global_gap']:>+9.4f}")

print(f"  {'─'*105}")

# Generative VLMs
print(f"  {'Qwen2.5-VL':<24s} {'Gen.VLM':<14s} {'3B':>6s} {'✗':>4s} {'✗':>4s} "
      f"{QWEN_NOREF['lp_f1_all']:>8.3f} {QWEN_NOREF['lp_f1_rare']:>8.3f} "
      f"{QWEN_NOREF['lp_auc']:>7.3f} {QWEN_NOREF['nn1']:>7.3f} {QWEN_NOREF['gap']:>+9.4f}")

print(f"  {'Qwen2.5-VL + Ref':<24s} {'Gen.VLM(DP)':<14s} {'3B':>6s} {'✗':>4s} {'✗':>4s} "
      f"{QWEN_REF['macro_all']['lp_f1']:>8.3f} {QWEN_REF['macro_rare']['lp_f1']:>8.3f} "
      f"{QWEN_REF['macro_all']['lp_auc']:>7.3f} {QWEN_REF['nn1_exact']:>7.3f} {QWEN_REF['global_gap']:>+9.4f}")

print(f"  {'MedGemma-4B + Ref':<24s} {'Gen.VLM(DP)':<14s} {'4B':>6s} {'✓':>4s} {'✗':>4s} "
      f"{MEDGEMMA['macro_all']['lp_f1']:>8.3f} {MEDGEMMA['macro_rare']['lp_f1']:>8.3f} "
      f"{MEDGEMMA['macro_all']['lp_auc']:>7.3f} {MEDGEMMA['nn1_exact']:>7.3f} {MEDGEMMA['global_gap']:>+9.4f}")

print(f"  {'Phi-3.5-Vision + Ref':<24s} {'Gen.VLM(DP)':<14s} {'4.2B':>6s} {'✗':>4s} {'✗':>4s} "
      f"{PHI['macro_all']['lp_f1']:>8.3f} {PHI['macro_rare']['lp_f1']:>8.3f} "
      f"{PHI['macro_all']['lp_auc']:>7.3f} {PHI['nn1_exact']:>7.3f} {PHI['global_gap']:>+9.4f}")

print(f"  {'─'*105}")

# LATEX
print(f"\n{'='*70}\n📋 LATEX — Per-class detail (BiomedCLIP)\n{'='*70}\n")

print("\\begin{table}[h]")
print("\\centering")
print("\\caption{Per-class embedding quality for BiomedCLIP (encoder-only, no references).}")
print("\\label{tab:emb_quality_biomedclip}")
...
summary = {
    ...
    'macro_all': BIOMEDCLIP['macro_all'],
    'macro_rare': BIOMEDCLIP['macro_rare'],
    'macro_common': BIOMEDCLIP['macro_common'],
    'per_class': results,
}
out_path = os.path.join(EMB_DIR, 'embedding_quality_summary.json')
