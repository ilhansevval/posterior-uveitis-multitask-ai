# ============================================================================
# PATOLOJI EMBEDDING QUALITY ANALYSIS — FULL (paper-ready)
# ============================================================================
# Eklemeler:
#   - Linear Probe AP (per-class)
#   - Centroid Acc (sırayla doğru kolonda)
#   - Rare-only macro (4 rare class average)
#   - F1 ± STD format
...
EMB_DIR = os.path.join(DATA_ROOT, 'teacher_embeddings_ref')
...
# ═══════════════════════════════════════════════════════════════
# Pairwise similarity (lazım)
# ═══════════════════════════════════════════════════════════════
sim_matrix = cosine_similarity(embs_norm)

# ═══════════════════════════════════════════════════════════════
# 1. MULTI-LABEL GAP (global)
# ═══════════════════════════════════════════════════════════════
print(f"\n{'='*70}\n📊 GLOBAL METRICS\n{'='*70}")
...
# ═══════════════════════════════════════════════════════════════
# 3. PER-CLASS METRICS (gap, NN-1, LP F1/AUC/AP, Centroid)
# ═══════════════════════════════════════════════════════════════
print(f"\n{'='*70}\n📊 PER-CLASS METRICS\n{'='*70}")
...
for i_lbl, sn in enumerate(ALL_PAT_SHORT):
    ...
    # --- Per-label intra/inter similarity ---
    pp_sims = []
    ...
    # --- Linear Probe (5-fold, patient-grouped) — F1, AUC, AP ---
    f1s, aucs, aps = [], [], []
    ...
    results[sn] = {
        'N': int(y.sum()),
        'pp_sim': float(pp_mean),
        'pn_sim': float(pn_mean),
        'gap': float(gap),
        'nn1': float(nn1_acc),
...
# ═══════════════════════════════════════════════════════════════
# 5. MACRO AVERAGES (all-8, rare-4, common-4)
# ═══════════════════════════════════════════════════════════════
def macro(keys, key):
...
# ═══════════════════════════════════════════════════════════════
# 6. PAPER TABLE (LaTeX-ready)
# ═══════════════════════════════════════════════════════════════
print(f"\n{'='*70}\n📋 LATEX TABLE (paper-ready)\n{'='*70}\n")

print("\\begin{table}[h]")
print("\\centering")
print("\\caption{Per-class embedding quality metrics for Qwen2.5-VL with reference images. "
      "LP: Linear Probe (5-fold patient-grouped CV). Cent.: centroid classification accuracy.}")
print("\\label{tab:emb_quality_qwen}")
...
print("\\end{tabular}")
print("\\\\[0.3em]")
print("\\footnotesize{$^{*}$Rare class (N $<$ 60). Global multi-label gap = "
      f"{multi_gap:+.4f}; NN-1 exact label-set match = {nn1_exact:.3f}.}}")
print("\\end{table}")

# ═══════════════════════════════════════════════════════════════
# 7. FOUNDATION MODEL COMPARISON ROW
# ═══════════════════════════════════════════════════════════════
print(f"\n{'='*70}\n📋 FOUNDATION MODEL COMPARISON — Qwen Row\n{'='*70}")
print(f"\n  Use this row when adding MedGemma, BiomedCLIP, RETFound, ImageNet:\n")

print(f"  Model         | Type      | Params | Med | Ret | LP F1 (all) | LP F1 (rare) | LP AUC | NN-1 exact")
print(f"  --------------|-----------|--------|-----|-----|-------------|--------------|--------|----------")
print(f"  Qwen2.5-VL+Ref| Gen.VLM   | 3B     |  ✗  |  ✗  | "
      f"{macro(all_keys,'lp_f1_mean'):.3f}       | "
      f"{macro(rare_keys,'lp_f1_mean'):.3f}        | "
      f"{macro(all_keys,'lp_auc'):.3f}  | {nn1_exact:.3f}")

# ═══════════════════════════════════════════════════════════════
# 8. SAVE JSON SUMMARY
# ═══════════════════════════════════════════════════════════════
summary = {
    'model': 'Qwen2.5-VL-Ref',
    ...
    'macro_all': {
        'gap': macro(all_keys, 'gap'),
        'nn1': macro(all_keys, 'nn1'),
        'lp_f1': macro(all_keys, 'lp_f1_mean'),
        'lp_auc': macro(all_keys, 'lp_auc'),
        'lp_ap': macro(all_keys, 'lp_ap'),
        'centroid_acc': macro(all_keys, 'centroid_acc'),
    },
    'macro_rare': {
        'gap': macro(rare_keys, 'gap'),
        'nn1': macro(rare_keys, 'nn1'),
        'lp_f1': macro(rare_keys, 'lp_f1_mean'),
        'lp_auc': macro(rare_keys, 'lp_auc'),
        'lp_ap': macro(rare_keys, 'lp_ap'),
        'centroid_acc': macro(rare_keys, 'centroid_acc'),
    },
    'macro_common': {
        'gap': macro(common_keys, 'gap'),
        'nn1': macro(common_keys, 'nn1'),
        'lp_f1': macro(common_keys, 'lp_f1_mean'),
        'lp_auc': macro(common_keys, 'lp_auc'),
        'lp_ap': macro(common_keys, 'lp_ap'),
        'centroid_acc': macro(common_keys, 'centroid_acc'),
    },
    'per_class': results,
}
