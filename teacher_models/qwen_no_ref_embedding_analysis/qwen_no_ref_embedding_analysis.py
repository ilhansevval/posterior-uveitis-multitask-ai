# ============================================================================
# PATOLOJI EMBEDDING QUALITY — NO-REFERENCE Qwen2.5-VL
# ============================================================================
# Same metrics as referanslı version, for fair comparison.
# Source: teacher_embeddings/ (referanssız)
# ============================================================================
...
# CONFIG — SADECE BURADA FARK: referans olmadan çıkarılan klasör
DATA_ROOT = r'C:\Users\gtu\Documents\cerrahpasa\files\files'
EMB_DIR = os.path.join(DATA_ROOT, 'teacher_embeddings')   # ★ ref'siz
...
# GLOBAL METRICS
# ============================================================================
print(f"\n{'='*70}\n📊 GLOBAL METRICS — Qwen2.5-VL (NO-REFERENCE)\n{'='*70}")

n = len(labels)
intra_sims, partial_sims, inter_sims = [], [], []
for i in range(n):
...
print(f"  Global NN-1 mean Jaccard:  {nn1_jaccard:.4f}")

# ============================================================================
# COMPARISON: REF vs NO-REF (önceki sonuçla side-by-side)
# ============================================================================
print(f"\n{'='*70}\n📊 NO-REF vs REF — Side-by-side comparison\n{'='*70}")

# Önceki (ref'li) sonuçlar — manuel girdi
REF_RESULTS = {
...
print(f"\n  {'Metric':<22s} {'No-Ref':>10s} {'Ref':>10s} {'Δ (Ref-NoRef)':>15s}")
print(f"  {'─'*60}")
print(f"  {'Global multi-gap':<22s} {NOREF_RESULTS['global_gap']:>+10.4f} "
      f"{REF_RESULTS['global_gap']:>+10.4f} {REF_RESULTS['global_gap']-NOREF_RESULTS['global_gap']:>+15.4f}")
print(f"  {'NN-1 exact match':<22s} {NOREF_RESULTS['nn1_exact']:>10.4f} "
      f"{REF_RESULTS['nn1_exact']:>10.4f} {REF_RESULTS['nn1_exact']-NOREF_RESULTS['nn1_exact']:>+15.4f}")
print(f"  {'─'*60}")
for grp_name, grp_key in [('Common', 'macro_common'), ('Rare', 'macro_rare'), ('All-8', 'macro_all')]:
    print(f"  -- {grp_name} --")
    for m in ['gap', 'nn1', 'lp_f1', 'lp_auc', 'lp_ap']:
        n_v = NOREF_RESULTS[grp_key][m]
        r_v = REF_RESULTS[grp_key][m]
        delta = r_v - n_v
        sign = "+" if delta > 0 else ""
        print(f"  {m:<22s} {n_v:>10.4f} {r_v:>10.4f} {sign}{delta:>+14.4f}")
    print()

# ============================================================================
# UNIFIED COMPARISON TABLE (paper format)
# ============================================================================
print(f"\n{'='*70}\n📋 PAPER TABLE — All Models Combined\n{'='*70}")
print(f"\n  Format: Foundation Model Comparison Table")
print()
print(f"  {'Model':<22s} {'Type':<12s} {'Param':>6s} {'Med':>4s} {'Ret':>4s} "
      f"{'F1-all':>8s} {'F1-rare':>8s} {'AUC':>7s} {'NN-1':>7s} {'Gap':>8s}")
print(f"  {'─'*100}")

# Qwen NO-REF
print(f"  {'Qwen2.5-VL':<22s} {'Gen.VLM':<12s} {'3B':>6s} {'✗':>4s} {'✗':>4s} "
      f"{NOREF_RESULTS['macro_all']['lp_f1']:>8.3f} "
      f"{NOREF_RESULTS['macro_rare']['lp_f1']:>8.3f} "
      f"{NOREF_RESULTS['macro_all']['lp_auc']:>7.3f} "
      f"{NOREF_RESULTS['nn1_exact']:>7.3f} "
      f"{NOREF_RESULTS['global_gap']:>+8.4f}")

# Qwen REF
print(f"  {'Qwen2.5-VL + Ref':<22s} {'Gen.VLM(DP)':<12s} {'3B':>6s} {'✗':>4s} {'✗':>4s} "
      f"{REF_RESULTS['macro_all']['lp_f1']:>8.3f} "
      f"{REF_RESULTS['macro_rare']['lp_f1']:>8.3f} "
      f"{REF_RESULTS['macro_all']['lp_auc']:>7.3f} "
      f"{REF_RESULTS['nn1_exact']:>7.3f} "
      f"{REF_RESULTS['global_gap']:>+8.4f}")

print(f"  {'─'*100}")
print(f"  {'MedGemma-4B':<22s} {'Gen.VLM':<12s} {'4B':>6s} {'✓':>4s} {'✗':>4s}     ...")
print(f"  {'BiomedCLIP':<22s} {'CLIP':<12s} {'400M':>6s} {'✓':>4s} {'✗':>4s}     ...")
print(f"  {'RETFound':<22s} {'SSL ViT':<12s} {'300M':>6s} {'✓':>4s} {'✓':>4s}     ...")

print(f"\n  Discriminative Prompting (DP): Reference images embedded in prompt.")

# ============================================================================
# LATEX TABLE
# ============================================================================
print(f"\n{'='*70}\n📋 LATEX — Per-class detail (Qwen No-Ref)\n{'='*70}\n")

print("\\begin{table}[h]")
print("\\centering")
print("\\caption{Per-class embedding quality for Qwen2.5-VL without reference images.}")
print("\\label{tab:emb_quality_qwen_noref}")
...
summary = {
    ...
    'macro_all': NOREF_RESULTS['macro_all'],
    'macro_rare': NOREF_RESULTS['macro_rare'],
    'macro_common': NOREF_RESULTS['macro_common'],
    'per_class': results,
}
