...
#   v3 KD kodu: aynı model, loss, sampler, scheduler, fold mantığı, eşikler.
#   Tek değişen: teacher embedding klasörü (döngü) + EMB_DIM otomatik +
...
#   MedGemma JSON'da zaten var -> atlanır; bu koşu sadece Phi'yi eğitir.
# ============================================================================
...
# MedGemma JSON'da var -> atlanır. Bu koşu Phi'yi eğitir.
TEACHERS = [
    ("MedGemma-4B + Ref",      "teacher_embeddings_medgemma"),  # bitti -> atlanır
    ("Phi-3.5-Vision + Ref",   "teacher_embeddings_phi"),       # ← bu koşacak
]
...
def remap_path(win_or_any_path):
    ...
    if len(parts) >= 2:
        return os.path.join(DATA_ROOT, parts[-2], parts[-1])
