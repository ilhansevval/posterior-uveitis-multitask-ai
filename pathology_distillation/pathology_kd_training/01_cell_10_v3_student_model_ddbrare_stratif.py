# ============================================================================
# CELL 10 v3: STUDENT MODEL — DDB→Rare, Stratified Folds, HEM fix
# ============================================================================
# Changes from v2:
#   - DDB moved to rare head (4 main + 4 rare)
#   - Custom stratified GroupKFold: rare labels guaranteed in every fold
#   - HEM threshold minimum 0.25 (prevent FP explosion)
#   - Per-rare-label early stopping (best checkpoint per head)
# ============================================================================
...
SAVE_DIR = os.path.join(DATA_ROOT, 'results_distillation_v3')
...
# Minimum thresholds for rare labels (prevent FP explosion)
...
def stratified_group_kfold(df, group_col, rare_cols, n_splits=5, random_state=42):
    """Sample-level balanced split: each label gets ~20% per fold, no zeros"""
    ...
    for col, _ in label_counts:
        # Positive samples for this label not yet assigned
        ...
        # Count already-assigned positives per fold
        ...
        # Distribute unassigned to balance across folds
        for idx in unassigned:
            target = int(np.argmin(already))
            fold_assign[idx] = target
            already[target] += 1
    
    # Assign remaining samples (all labels = 0) to smallest folds
...
# Verify distribution
print(f"\n📊 Fold distribution (sample-level balanced):")
...
# ─────────────────────────────────────────────
# MODEL — 4 main + 4 separate rare heads
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
print(f"\n{'=' * 80}")
print(f"🚀 STARTING {NUM_FOLDS}-FOLD TRAINING (v3)")
print(f"{'=' * 80}")
...
for fold, (train_idx, val_idx) in enumerate(splits):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # Use min thresholds for monitoring
...
# ═══════════════════════════════════════════════════════════════
print(f"\n{'=' * 80}")
print(f"📊 GLOBAL RESULTS — {NUM_FOLDS}-Fold CV (v3)")
print(f"{'=' * 80}")
...
gmacro = print_table(gm, title="GLOBAL METRICS (v3)")
print(f"  Thresholds: {' '.join([f'{ALL_SHORT[i]}={g_thresh[i]:.3f}' for i in range(NUM_CLASSES)])}")

# Confusion matrix
print(f"\n  Confusion Matrix:")
print(f"  {'GT\\Pred':10s} " + " ".join([f"{sn:>6s}" for sn in ALL_SHORT]))
print(f"  {'─' * (10 + 7*NUM_CLASSES)}")
for i, sn_i in enumerate(ALL_SHORT):
    gt_mask = gt[:, i] == 1
    counts = [int(gd[gt_mask, j].sum()) for j in range(NUM_CLASSES)]
    print(f"  {sn_i:10s} " + " ".join([f"{c:6d}" for c in counts]) + f"  (N={int(gt_mask.sum())})")
...
# ── COMPARISON ──
print(f"\n{'=' * 80}")
print(f"📊 COMPARISON — All versions")
print(f"{'=' * 80}")

cascade = {'DKS':0.7437,'ODB':0.7226,'VI':0.5362,'MÖ':0.5641,'DDB':0.3108,'RI':0,'HEM':0,'PVK':0}
uni_v2 = {'DKS':0.69,'ODB':0.74,'VI':0.48,'MÖ':0.53,'DDB':0.38,'RI':0,'HEM':0,'PVK':0}
dist_v1 = {'DKS':0.8983,'ODB':0.9032,'VI':0.5660,'MÖ':0.7619,'DDB':0.4348,'RI':0,'HEM':0,'PVK':0.2857}
dist_v2 = {'DKS':0.8571,'ODB':0.8679,'VI':0.6437,'MÖ':0.6772,'DDB':0.3559,'RI':0.7317,'HEM':0.1649,'PVK':0.5500}

f1s = [r['F1'] for r in gm]

print(f"\n  {'Label':6s} {'N':>4s} {'Cascade':>8s} {'Uni_v2':>8s} {'Dst_v1':>8s} {'Dst_v2':>8s} {'Dst_v3':>8s} {'Best':>6s}")
print(f"  {'─' * 62}")
for i, sn in enumerate(ALL_SHORT):
    c=cascade.get(sn,0); u=uni_v2.get(sn,0); d1=dist_v1.get(sn,0); d2=dist_v2.get(sn,0); d3=f1s[i]
    best = max(c,u,d1,d2,d3)
    mark = "v3" if d3 == best else ("v2" if d2 == best else ("v1" if d1 == best else "prev"))
    print(f"  {sn:6s} {gm[i]['N']:4d} {c:8.4f} {u:8.4f} {d1:8.4f} {d2:8.4f} {d3:8.4f} {mark:>6s}")

macro = np.mean(f1s)
print(f"  {'─' * 62}")
print(f"  {'MACRO':6s} {'':4s} {'—':>8s} {'0.470':>8s} {'0.385':>8s} {'0.606':>8s} {macro:8.4f}")
