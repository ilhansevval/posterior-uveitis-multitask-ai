# ============================================================================
# CELL 9: BATCH EMBEDDING EXTRACTION — 561 pathological images → .npy
# ============================================================================
# Uses: vl_model, vl_processor, df_pat, make_prompt, SYSTEM_PROMPT from Cell 8b
# Extracts reasoning embeddings WITHOUT GT guidance (last_30 strategy, max_tokens=1000)
# GT labels are stored as metadata only — they are NOT fed to the model
...
SAVE_DIR = os.path.join(DATA_ROOT, 'teacher_embeddings')
...
print(f"   Estimated time: ~{len(remaining) * 15 / 60:.0f} min\n")

# ── Extraction function (last_30 strategy, max_tokens=1000, no GT guidance) ──
def extract_teacher_embedding(img_path):
...
for i, row in remaining.iterrows():
    ...
    # gt_dict is used ONLY for metadata/logging — it is NOT fed to the model
    ...
    try:
        emb, expl = extract_teacher_embedding(img_path)
