#!/usr/bin/env python3
"""
🏥 FUNDUS PATHOLOGY — EfficientNet-B4 vanilla BASELINE (KD'siz)
...
class Config:
    ...
    RESULTS_DIR = os.path.join(DATA_ROOT, 'results_effnet_vanilla_baseline')
...
# ============================================================================
# MODEL — EfficientNet-B4 vanilla (vanilla, düz classifier)
# ============================================================================
class EffNetVanillaBaseline(nn.Module):
...
def main():
    ...
    print("=" * 80)
    print("🏥 EfficientNet-B4 vanilla BASELINE (KD'siz) — 8 Label")
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(fold_splits):
        ...
        model = EffNetVanillaBaseline(cfg.N_LABELS).to(device)
    ...
    # ── Final ──
    print(f"\n{'='*80}\n📊 FINAL SONUÇLAR — EfficientNet-B4 vanilla 5-Fold CV\n{'='*80}")
    ...
    print(f"\n  ── KARŞILAŞTIRMA ──")
    print(f"  {'EffNet-B4 vanilla':18s} mF1={macro['f1']:.4f}  rareF1={rare['f1']:.4f}")
    ...
    # Save
    save = {
        'model': 'EfficientNet-B4 vanilla (no KD)', 'macro_f1': float(macro['f1']),
    ...
    with open(results_dir / 'effnet_vanilla_summary.json', 'w', encoding='utf-8') as f:
    ...
    pred_df.to_csv(results_dir / 'predictions_effnet_vanilla.csv', index=False)

    print(f"\n  💾 {results_dir}/")
    print(f"  ⏱ {elapsed:.1f} dk")
    print(f"\n{'='*80}\n✅ EfficientNet-B4 vanilla BASELINE TAMAMLANDI — mF1={macro['f1']:.4f}\n{'='*80}")
