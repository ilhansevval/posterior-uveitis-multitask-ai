#!/usr/bin/env python3
"""
🏥 FUNDUS PATHOLOGY — BiomedCLIP BASELINE (frozen feature extractor)
   BiomedCLIP (ViT-B/16 image encoder, PubMedBERT text — vision-language FM)
   pretrained on PMC-15M (15M biomedical figure-caption pairs)
   → frozen 512-dim image embedding → trainable classifier head
   Aynı 5-fold split, aynı 8 label, aynı threshold optimization, aynı metrics

   BiomedCLIP: open_clip ile yüklenir (microsoft repo, açık erişim, form gerekmez).
...
class Config:
    ...
    RESULTS_DIR = os.path.join(DATA_ROOT, 'results_biomedclip_baseline')

    # BiomedCLIP (open_clip, açık erişim)
    OPENCLIP_MODEL = 'hf-hub:microsoft/BiomedCLIP-PubMedBERT_256-vit_base_patch16_224'
    IMG_SIZE = 224
    BATCH_SIZE = 32
    NUM_WORKERS = 0
...
# ============================================================================
# BiomedCLIP FEATURE EXTRACTION (frozen)
# ============================================================================
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pixel = self.preprocess(img)   # open_clip preprocess → tensor
        return pixel, idx


@torch.no_grad()
def extract_biomedclip_features(df, device):
    """BiomedCLIP ViT-B/16 image embeddings (512-dim)."""
    try:
        import open_clip
    except ImportError:
    ...
    print(f"  BiomedCLIP yükleniyor: {Config.OPENCLIP_MODEL} ...")
    model, preprocess = open_clip.create_model_from_pretrained(Config.OPENCLIP_MODEL)
    ...
    # probe feature dim with a dummy
    feats = None
    print(f"  Feature extraction ({len(df)} imgs)...")
    t0 = time.time()
    for pixels, idxs in tqdm(loader, desc="  BiomedCLIP feat", bar_format='{l_bar}{bar:30}{r_bar}', leave=False):
        pixels = pixels.to(device)
        emb = model.encode_image(pixels)         # (B, 512)
        emb = emb.float().cpu().numpy()
        if feats is None:
            feats = np.zeros((len(df), emb.shape[1]), dtype=np.float32)
        feats[idxs.numpy()] = emb
    print(f"  ✓ Features extracted in {(time.time()-t0)/60:.1f} min (dim={feats.shape[1]})")
    return feats
...
def main():
    ...
    print(f"   Device: {device} | Model: {cfg.OPENCLIP_MODEL}")
    print("=" * 80)
    ...
    feat_cache = results_dir / 'biomedclip_features.npy'
    if feat_cache.exists():
        features = np.load(feat_cache)
        print(f"  ✓ Cached features: {features.shape}")
    else:
        features = extract_biomedclip_features(df, device)
    ...
    print(f"\n{'='*80}\n📊 FINAL — BiomedCLIP ({best_name} seçildi)\n{'='*80}")
    ...
    print(f"\n  ── İki yöntem ──")
    print(f"  BiomedCLIP Linear Probe: mF1={m_lp['__macro__']['f1']:.4f} rareF1={m_lp['__rare__']['f1']:.4f}")
    print(f"  BiomedCLIP MLP Head:     mF1={m_mlp['__macro__']['f1']:.4f} rareF1={m_mlp['__rare__']['f1']:.4f}")

    print(f"\n  ── KARŞILAŞTIRMA (TAM TABLO) ──")
    print(f"  {'BiomedCLIP (best)':18s} mF1={macro['f1']:.4f}  rareF1={rare['f1']:.4f}")
    print(f"  {'RETFound (frozen)':18s} mF1=0.4095  rareF1=0.3213")
    print(f"  {'XGBoost':18s} mF1=0.3637  rareF1=0.1899")
    print(f"  {'RandomForest':18s} mF1=0.3686  rareF1=0.1847")
    print(f"  {'SVM':18s} mF1=0.3749  rareF1=0.1957")
    print(f"  {'LogReg':18s} mF1=0.3975  rareF1=0.2092")
    ...
    save = {
        'model': f'BiomedCLIP frozen ({best_name})',
    ...
    with open(results_dir / 'biomedclip_summary.json', 'w', encoding='utf-8') as f:
        json.dump(save, f, indent=2, ensure_ascii=False)
    print(f"\n  💾 {results_dir}/biomedclip_summary.json")
    print(f"\n{'='*80}\n✅ BiomedCLIP BASELINE TAMAMLANDI — mF1={macro['f1']:.4f}\n{'='*80}")
