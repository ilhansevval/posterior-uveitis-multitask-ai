#!/usr/bin/env python3
"""
🏥 FUNDUS PATHOLOGY — ViT-Base/16 BASELINE (KD'siz)
...
class Config:
    ...
    RESULTS_DIR = os.path.join(DATA_ROOT, 'results_vit_baseline')
...
# ============================================================================
# MODEL — ViT-Base/16 (vanilla, düz classifier)
# ============================================================================
class ViTBaseline(nn.Module):
    def __init__(self, n_labels=8):
        ...
        n_features = self.backbone.num_features  # 768
...
def main():
    ...
    print("=" * 80)
    print("🏥 ViT-Base/16 BASELINE (KD'siz) — 8 Label")
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(fold_splits):
        ...
        model = ViTBaseline(cfg.N_LABELS).to(device)
    ...
    # ── Final ──
    print(f"\n{'='*80}\n📊 FINAL SONUÇLAR — ViT-Base/16 5-Fold CV\n{'='*80}")
    ...
    print(f"\n  ── KARŞILAŞTIRMA ──")
    print(f"  {'ViT-Base/16 (noKD)':18s} mF1={macro['f1']:.4f}  rareF1={rare['f1']:.4f}")
    ...
    # Save
    save = {
        'model': 'ViT-Base/16 (no KD)', 'macro_f1': float(macro['f1']),
    ...
    with open(results_dir / 'vit_summary.json', 'w', encoding='utf-8') as f:
    ...
    pred_df.to_csv(results_dir / 'predictions_vit.csv', index=False)

    print(f"\n  💾 {results_dir}/")
    print(f"  ⏱ {elapsed:.1f} dk")
    print(f"\n{'='*80}\n✅ ViT-Base/16 BASELINE TAMAMLANDI — mF1={macro['f1']:.4f}\n{'='*80}")
