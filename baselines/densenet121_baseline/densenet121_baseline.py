#!/usr/bin/env python3
"""
🏥 FUNDUS PATHOLOGY — DenseNet-121 BASELINE (KD'siz)
...
class Config:
    ...
    RESULTS_DIR = os.path.join(DATA_ROOT, 'results_densenet121_baseline')
...
# ============================================================================
# MODEL — DenseNet-121 (vanilla, düz classifier)
# ============================================================================
class DenseNet121Baseline(nn.Module):
    def __init__(self, n_labels=8):
        ...
        n_features = self.backbone.num_features  # 1024
...
def main():
    ...
    print("=" * 80)
    print("🏥 DenseNet-121 BASELINE (KD'siz) — 8 Label")
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(fold_splits):
        ...
        model = DenseNet121Baseline(cfg.N_LABELS).to(device)
    ...
    # ── Final ──
    print(f"\n{'='*80}\n📊 FINAL SONUÇLAR — DenseNet-121 5-Fold CV\n{'='*80}")
    ...
    print(f"\n  ── KARŞILAŞTIRMA ──")
    print(f"  {'DenseNet-121 (noKD)':18s} mF1={macro['f1']:.4f}  rareF1={rare['f1']:.4f}")
    ...
    # Save
    save = {
        'model': 'DenseNet-121 (no KD)', 'macro_f1': float(macro['f1']),
    ...
    with open(results_dir / 'densenet121_summary.json', 'w', encoding='utf-8') as f:
    ...
    pred_df.to_csv(results_dir / 'predictions_densenet121.csv', index=False)

    print(f"\n  💾 {results_dir}/")
    print(f"  ⏱ {elapsed:.1f} dk")
    print(f"\n{'='*80}\n✅ DenseNet-121 BASELINE TAMAMLANDI — mF1={macro['f1']:.4f}\n{'='*80}")
